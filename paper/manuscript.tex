...
\title{A Reproducible Support-Boundary Diagnostic for Tabular--Neural
...
\begin{abstract}
Hybrid reinforcement-learning controllers that combine exact-state memory with
neural value functions must decide when memorized evidence is reliable and when
neural extrapolation is being used outside observed support. This article
presents a reproducible support-boundary diagnostic for tabular--neural
Q-learning rather than a claim of universal algorithmic superiority. The study
uses count gating, fixed mixing, support abstention, an approximate-support
comparator, and a fuzzy support- and TD-residual-aware arbitration baseline. All
experiments retain seed-level outcomes, exact environment-step checkpoints,
read-only evaluation, isolated evaluation randomness, provenance records, and
executable audits. In compact recurrent tasks, count gating improves over the
validated DQN on FrozenLake 4x4, FrozenLake 8x8, and CliffWalking, but it does
not generally outperform tabular Q-learning. Under complete held-out exact-state
support in FourRooms, count and fuzzy gates degenerate to neural fallback and
fail consistently, whereas support abstention repairs the specific unsupported
preference mechanism. A 30-seed application-inspired navigation diagnostic with
deployment goal shift shows the same trade-off: fuzzy arbitration improves AUC
over the validated DQN by 0.575 (bootstrap 95\% interval [0.152, 0.987]) but is
worse than tabular Q-learning and support abstention, while abstention sharply
reduces collisions by selecting a hold fallback at unsupported states. The main
contribution is an auditable pre-deployment diagnostic: exact memory helps when
states recur, adaptive arbitration helps only when its reliability signals are
informative, and explicit fallback remains necessary at the support boundary.
\end{abstract}

\begin{keyword}
reinforcement learning \sep support-boundary diagnostic \sep fuzzy arbitration \sep
...
The contributions are deliberately limited to five auditable claims:
\begin{enumerate}
  \item a reproducible support-boundary evaluation protocol for tabular--neural
  mixtures with exact interaction budgets, read-only evaluation, isolated
  random-number streams, raw seed retention, provenance, and executable audits;
  \item confirmatory evidence that count gating can improve a
  development-selected and independently validated DQN only when exact states
  recur;
  \item held-out-support evidence and proof of zero-support degeneration showing
  why count and fuzzy gates can fail when exact memory has no evidence;
  \item an independently seeded confirmation-after-discovery replication showing
  that support abstention repairs that specific unsupported-neural-preference
  mechanism without becoming a general transfer method; and
  \item a controlled application-inspired navigation diagnostic that reports
  return, success, collision, risk, unsupported-state exposure, branch use,
  abstention, exact-memory cost, and per-decision timing.
\end{enumerate}
...
\cite{vanhasselt2016double}. The statistical claims are intentionally restricted
to this audited comparator family. The artifact also contains executable
dueling Double-DQN and online advantage actor-critic (A2C) implementations,
together with reviewer-response configuration files for full follow-up runs;
those code paths are not used to claim stronger-neural-baseline performance in
this article because the deposited results contain only smoke validation for
them.
...
choice. The released package also contains auxiliary dependency-free smoke
validations for the approximate-support gate, fuzzy-parameter variants, and the
dueling-DQN/A2C code paths. They are archived with the artifact as code-path
checks only and are separated from the eight main evidence families in
Table~\ref{tab:benchmark-status}.
...
\centering
\small
\caption{Eight main executed ASOC result families. All listed audits passed; auxiliary smoke checks are archived in the artifact but not used as performance evidence.}
\label{tab:benchmark-status}
\resizebox{\textwidth}{!}{\input{generated/asoc_benchmark_status.tex}}
\end{table*}
...
\resizebox{\textwidth}{!}{\input{generated/asoc_recurrent_confirmatory.tex}}
\end{table*}
...
\resizebox{\textwidth}{!}{\input{generated/asoc_support_replication.tex}}
\end{table*}
...
Accordingly, the comparator is more extensively validated than the original
choice, but the study does not claim a statistically established universal DQN
improvement. A stricter acceptance criterion would require full 30-seed
dueling-DQN, Double-DQN, A2C or PPO, and preferably convolutional MiniGrid
comparators; because those full runs are not in the deposited evidence, all
hybrid-versus-neural statements are phrased relative to the validated DQN used
here.
...
\resizebox{\textwidth}{!}{\input{generated/asoc_dqn_validation.tex}}
\end{table*}
...
\label{tab:minigrid}
\resizebox{\textwidth}{!}{\input{generated/asoc_minigrid_extended.tex}}
\end{table*}
...
0.498 success but has 0.244 collision rate. Risk-zone occupancy is lower than
collision exposure for all methods, but it follows the same cautionary message:
return, success, collision, risk, and abstention must be read jointly. These
results support a decision-support interpretation: a safe fallback can reduce
harmful motion without creating transfer, whereas neural fallback can improve
some learning comparisons while retaining deployment risk.
...
final-checkpoint seed means; AUC intervals are seed-bootstrap 95\% intervals.}
\label{tab:application-case}
\resizebox{\textwidth}{!}{\input{generated/asoc_application_case_study.tex}}
\end{table*}
\FloatBarrier
...
\resizebox{\textwidth}{!}{\input{generated/asoc_adaptive_gate_summary.tex}}
\end{table*}
...
\resizebox{\textwidth}{!}{\input{generated/asoc_cost_and_support_metrics.tex}}
\end{table*}
...
Auxiliary smoke validations for approximate support, fuzzy parameter variants,
and dueling-DQN/A2C executability are included in the artifact rather than in
the main results. They are useful for release auditing but are not performance
evidence and should not be read as substitutes for full confirmatory runs.
...
table support under the deliberately strict key definition, and the standard
count gate responds by placing more weight on the neural estimator. The
three-size replication shows that this design can be systematically harmful.
Support abstention repairs the particular failure because it refuses unsupported
neural preferences, but it does not create positive transfer and does not
generally exceed tabular learning. This is also why the approximate-support
comparator is separated from the exact-support claims: nearest-neighbor support
may soften the boundary, but it must be evaluated as a different assumption,
not retrofitted into the exact-key result.
...
success relative to tabular learning. Because hold is deliberately safe in this
controlled map, the collision reduction is a diagnostic result rather than a
field-safety proof. Fuzzy and count gates continue to move, which can improve
learning relative to DQN but can also raise collision exposure. An
operator-facing system could use the logged support signal to request evidence,
hand control to a verified planner, or hold position. The present experiment
measures that decision boundary; it does not validate any particular
...
The revised code now includes dueling-DQN and A2C agents plus full-run
configuration templates for reviewer-requested follow-up studies. Because the
deposited stronger-baseline results are intentionally small and run in the
dependency-free fallback environment, they verify executability but do not
support broad performance claims about stronger neural baselines. A future
acceptance-oriented revision should execute those comparators, and preferably
PPO and convolutional MiniGrid models, at the same seed scale as the main
families.
...
and a full-run configuration template; that limited deposited result confirms
that nearby memory can be queried, but it is not a full-scale performance
study. Computational limits prevented investigation of
...
\cite{erkalkan2026artifact}. If a journal-specific Zenodo release is minted,
the version-specific DOI for that release should replace the concept DOI in the
final proofs. The supplementary artifact submitted with this manuscript contains
the exact code, configurations, seed-level results, tests, generated assets, and
audit records used to produce the reported claims.
\endgroup
