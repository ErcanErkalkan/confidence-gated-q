...
def _finite_vector(values: object, *, name: str = "values") -> np.ndarray:
    """Return finite values from a one-dimensional numeric input.

    NaN and positive/negative infinity are excluded deliberately.  The caller
    receives a clear error when the input is not one-dimensional or contains no
    finite observation, so an unavailable metric cannot silently become a
    fabricated zero.
    """
    ...
    if isinstance(values, np.ndarray):
        ...
        try:
            array = np.asarray(list(values), dtype=float)  # type: ignore[arg-type]
        except TypeError:
            array = np.asarray(values, dtype=float)
    if array.ndim != 1:
...
def empirical_lower_cvar(values: object, alpha: float) -> float:
    """Return the locked empirical lower-tail CVaR estimator.

    The estimator is the arithmetic mean of the lowest
    ``max(1, ceil(alpha * n))`` finite observations.  Non-finite observations
    are excluded, ties are retained according to their multiplicity, and a
    single finite observation therefore returns that observation.  ``alpha``
    must lie in ``(0, 1]``.
    """

    if not np.isfinite(alpha) or not 0.0 < float(alpha) <= 1.0:
        raise ValueError("alpha must be finite and lie in (0, 1]")
    finite = np.sort(_finite_vector(values))
    tail_count = max(1, int(np.ceil(float(alpha) * finite.size)))
    return float(finite[:tail_count].mean())


def lower_quantile(values: object, q: float) -> float:
    """Return the linear empirical quantile of the finite observations.

    NaN and infinity are excluded.  ``q`` must lie in ``[0, 1]``.  NumPy's
    linear interpolation is used explicitly; for ``n=1`` the sole observation
    is returned.
    """

    if not np.isfinite(q) or not 0.0 <= float(q) <= 1.0:
        raise ValueError("q must be finite and lie in [0, 1]")
    return float(np.quantile(_finite_vector(values), float(q), method="linear"))


def worst_decile_mean(values: object) -> float:
    """Return the empirical mean of the lowest ten percent of finite values."""

    return empirical_lower_cvar(values, 0.10)


def maximum_drawdown(checkpoints: object, values: object) -> float:
    """Return the largest peak-to-later-trough decline in value units.

    Checkpoint/value pairs with a non-finite member are excluded together.
    Remaining checkpoints are sorted in ascending order and must be unique.
    Tied values are valid, and a one-checkpoint curve has drawdown zero.
    """

    checkpoint_array = np.asarray(checkpoints, dtype=float)
    value_array = np.asarray(values, dtype=float)
    if checkpoint_array.ndim != 1 or value_array.ndim != 1:
        raise ValueError("checkpoints and values must be one-dimensional")
    if checkpoint_array.size != value_array.size:
        raise ValueError("checkpoints and values must have equal length")
    finite = np.isfinite(checkpoint_array) & np.isfinite(value_array)
    checkpoint_array = checkpoint_array[finite]
    value_array = value_array[finite]
    if checkpoint_array.size == 0:
        raise ValueError("curve must contain at least one finite checkpoint/value pair")
    order = np.argsort(checkpoint_array, kind="stable")
    checkpoint_array = checkpoint_array[order]
    value_array = value_array[order]
    if np.unique(checkpoint_array).size != checkpoint_array.size:
        raise ValueError("finite checkpoints must be unique")
    running_peak = np.maximum.accumulate(value_array)
    return float(np.max(running_peak - value_array))


def worst_checkpoint(values: object) -> float:
    """Return the minimum finite checkpoint value."""

    return float(np.min(_finite_vector(values)))
...
def seed_level_metrics(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = (
        ...
        .agg(
            ...
            training_loss_mean=("training_loss_mean", "max"),
            training_loss_max=("training_loss_max", "max"),
            nonfinite_loss_count=("nonfinite_loss_count", "max"),
            completed_checkpoint_count=(
                "completed_checkpoint_count",
                "max",
            ),
            expected_checkpoint_count=("expected_checkpoint_count", "max"),
            training_elapsed_seconds=(
                "training_elapsed_seconds",
                "max",
            ),
            ...
            exact_support_coverage=("exact_support_coverage", "mean"),
            approximate_support_coverage=(
                "approximate_support_coverage",
                "mean",
            ),
            fuzzy_gate_active_rate=("fuzzy_gate_active_rate", "mean"),
            neural_only_fallback_rate=("neural_only_fallback_rate", "mean"),
            out_of_fuzzy_support_count=(
                "out_of_fuzzy_support_count",
                "mean",
            ),
