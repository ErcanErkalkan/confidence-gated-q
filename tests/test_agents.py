...
def test_fuzzy_gate_can_use_neural_fallback_outside_exact_support():
    ...
    assert decision["fuzzy_gate_active"] == 0.0
    assert decision["support_boundary_collapse"] == 1.0
    assert decision["support_status"] == "support-boundary collapse"
...
def test_fuzzy_risk_gate_records_branch_diagnostics_without_changing_values():
    agent = HybridQAgent(
        input_dim=2,
        action_dim=2,
        seed=0,
    ...
    agent.online = FixedQNetwork([0.25, 0.75])
    agent.table["s"] = np.array([1.0, 0.0], dtype=np.float32)
    agent.counts["s"] = 10
    agent.error_counts["s"] = 10
    agent.tabular_error["s"] = 0.2
    agent.neural_error["s"] = 0.8
    state = np.array([1.0, 0.0], dtype=np.float32)

    values = agent.q_values(state, "s")
    decision = agent.decision_diagnostics()

    expected_alpha = agent.fuzzy_reliability_components("s")[0]
    expected = expected_alpha * agent.table["s"] + (1.0 - expected_alpha) * np.array(
        [0.25, 0.75], dtype=np.float32
    )
    assert np.allclose(values, expected)
    assert decision["tabular_greedy_action"] == 0
    assert decision["neural_greedy_action"] == 1
    assert np.isfinite(decision["relative_reliability_score"])
    assert decision["predicted_better_branch"] == "tabular"
...
def test_approximate_support_gate_retrieves_nearby_memory():
    ...
    decision = agent.decision_diagnostics()
    assert decision["unsupported_state"] == 1.0
    assert decision["approximate_support"] == 1.0
    assert decision["memory_branch_weight"] > 0.0
    assert values[1] > values[0]


def test_noisy_22d_state_is_exactly_unsupported_but_binned_supported():
    agent = HybridQAgent(
        input_dim=22,
        action_dim=3,
        seed=0,
        config=AgentConfig(
            ...
            support_diagnostic_bin_width=0.10,
        ),
        gate_kind="fuzzy_support_adaptive",
    )
    ...
    nearby = state.copy()
    nearby[0] = 0.02
    next_state = np.ones(22, dtype=np.float32)
    agent.observe(state, ("exact", 0), 1, 1.0, next_state, "next", True)
    agent.q_values(nearby, ("noisy", 1))
    decision = agent.decision_diagnostics()
    assert decision["exact_support"] == 0.0
    assert decision["approximate_support"] == 1.0
    assert decision["fuzzy_gate_active"] == 0.0
    assert decision["neural_only_fallback"] == 1.0
    assert decision["support_status"] == "support-boundary collapse"
